...
<h2>Beautiful Soup: <code>NavigableString</code></h2>\
<p>\
Let us start from Beautiful Soup. \
...
<h2>PyYAML: <code>safe_dump</code></h2>\
<p>\
...
`
    const emit = mldown(src, new HtmlTransor()).replace(/ >/g, '>')
    expect(emit).toEqual(html)
  })
})
...
\`\`\`
`
    const emit = mldown(src, new MaterialUITransor()).replace(/ >/g, '>')
    const jsx = `<Typography component="h2">Problem</Typography><p>Today I am writting a Telegram bot.{'\\n'}I choose YAML to display the results for users.{'\\n'}The results are requested by <Box component="code" fontFamily="Source Code Pro">requests</Box>, parsed by <Box component="code" fontFamily="Source Code Pro">Beautiful Soup</Box>, and finally dumped by <Box component="code" fontFamily="Source Code Pro">PyYAML</Box>, which is a common plan.{'\\n'}However, when testing, a strange error comes out:</p><pre><Box component="code" fontFamily="Source Code Pro"># Tons of traceback info, all from \`yaml/representer.py\`.{'\\n'}RecursionError: maximum recursion depth exceeded in __instancecheck__{'\\n'}</Box></pre><p>But the dumped Python dict is just simple, and you can easily copy it and dump it in a Python shell.</p><p>After a long time web searching, I finally realize what I have done.</p><Typography component="h2">Beautiful Soup: <Box component="code" fontFamily="Source Code Pro">NavigableString</Box></Typography><p>Let us start from Beautiful Soup.{'\\n'}I find out that I have used <Box component="code" fontFamily="Source Code Pro">tag.string</Box> to get the text node in HTML, and pass it to PyYAML to dump it to readable text.{'\\n'}Here is a point: <Box fontWeight="fontWeightBold">Beautiful Soup ,<Box component="code" fontFamily="Source Code Pro">tag.string</Box>, returns ,<Box component="code" fontFamily="Source Code Pro">NavigableString</Box>,, not Python builti-in ,<Box component="code" fontFamily="Source Code Pro">str</Box>,.</Box>{'\\n'}The <Box component="code" fontFamily="Source Code Pro">NavigableString</Box> carries a reference to the entire Beautiful Soup parse tree, and provides some extra methods that working on it.{'\\n'}The original documentation text from <Link href="https://www.crummy.com/software/BeautifulSoup/bs4/doc/#navigablestring">here</Link> is:</p><blockquote><p>If you want to use a <Box component="code" fontFamily="Source Code Pro">NavigableString</Box> outside of Beautiful Soup, you should call <Box component="code" fontFamily="Source Code Pro">unicode()</Box> on it to turn it into a normal Python Unicode string.{'\\n'}If you don’t, your string will carry around a reference to the entire Beautiful Soup parse tree, even when you’re done using Beautiful Soup.{'\\n'}This is a big waste of memory.</p></blockquote><p>So we should call <Box component="code" fontFamily="Source Code Pro">str()</Box> to convert <Box component="code" fontFamily="Source Code Pro">NavigableString</Box> to <Box component="code" fontFamily="Source Code Pro">str</Box>.</p><Typography component="h2">PyYAML: <Box component="code" fontFamily="Source Code Pro">safe_dump</Box></Typography><p>As for PyYAML, <Link href="https://pyyaml.org/wiki/PyYAMLDocumentation">the documentation</Link> is not good.{'\\n'}As we (may) know YAML can work on complicated Python objects, and the libraries usually provides an additional set of safe functions, which avoids this.{'\\n'}PyYAML safe dump function can be found <Link href="https://pyyaml.org/wiki/PyYAMLDocumentation#reference">here</Link> and <Link href="https://pyyaml.org/wiki/PyYAMLDocumentation#dumper">here</Link>, which are at the nearly bottom, and not shown in the quick start.{'\\n'}Here is a useful text:</p><blockquote><p><Box component="code" fontFamily="Source Code Pro">SafeDumper(stream)</Box> produces only standard YAML tags and thus cannot represent class instances and probably more compatible with other YAML processors.{'\\n'}The functions <Box component="code" fontFamily="Source Code Pro">safe_dump</Box> and <Box component="code" fontFamily="Source Code Pro">safe_dump_all</Box> use <Box component="code" fontFamily="Source Code Pro">SafeDumper</Box> to produce a YAML document.</p></blockquote><p>(Additionally, use <Box component="code" fontFamily="Source Code Pro">CDumper</Box> to get a lot better preformance.){'\\n'}I found I have used <Box component="code" fontFamily="Source Code Pro">yaml.dump()</Box>, so the PyYAML try to dump the <Box component="code" fontFamily="Source Code Pro">NavigableString</Box>, and as it contains a reference to the entire Beautiful Soup parse tree, and I have used <Box component="code" fontFamily="Source Code Pro">tag.string</Box> many times, the PyYAML is trying to dump the entire tree many times!{'\\n'}Easy to guess it reaches the recusive depth limit, which is 1000 without editing.</p><p>After fixing the above mistakes, Everything works fine.</p><Typography component="h2">Additionally</Typography><p>To get current recusive depth and the limit of it, and mute the very very long recusive error traceback info, I have searched the web, finding them on Stack Overflow.{'\\n'}Here are them:</p><pre><Box component="code" fontFamily="Source Code Pro" class="language-python"># Limit traceback depth.{'\\n'}import sys{'\\n'}sys.tracebacklimit = 1{'\\n'}# Get the recusive depth limit.{'\\n'}sys.getrecursionlimit(){'\\n'}# Set it, default 1000 usually.{'\\n'}sys.getrecursionlimit(1000){'\\n'}# Get current recusive depth.{'\\n'}import inspect{'\\n'}len(inspect.stack()){'\\n'}</Box></pre>`
    expect(emit).toEqual(jsx)
  })
})
